...
import { FaMoneyBillTrendUp, FaHospital, FaFilm, FaPhone, FaTruck, FaSellsy } from 'react-icons/fa6'
...
function IndustryCard({ text1, text2, icon }: { text1: string, text2: string, icon: IconType }) {
    return (
        <Box px={4} my={1}>
        ...
        <Box id='Industries'>
            ...
            <Box display={{ base: 'none', lg: 'block' }} >
                <SimpleSlider slidesToShow={4} horizontalMargin={12} arrows={true}>
                    {data.map(item => (<IndustryCard text1={item.title} text2={item.desc} icon={item.icon} />))}
                </SimpleSlider>
            </Box>
